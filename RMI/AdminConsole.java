    ...
    try {
        *chamada de um método rmis*
        ...
        while(contador<30) {
            try {
                Thread.sleep(1000);
                rmis = (RMI.RMIServer_I) LocateRegistry.getRegistry(6969).lookup(lookupString);
                rmis.sayHello();
                break;
...
public class AdminConsole extends UnicastRemoteObject implements AdminConsole_I {
    ...
    public static void main(String[] args) {
        // Usage javac RMI.AdminConsole.java && java RMI.AdminConsole endereço_ip
        ...
        while (true) {
            try {
                ...
                System.out.println("Client sent subscription to RMI.RMIServer");

                do {
                    System.out.println("------------MENU PRINCIPAL------------\n"
                            + "* ESCOLHA UMA DAS OPCOES DISPONIVEIS *\n" + "______________________________________\n"
                            + "1.Registar RMI.Pessoa\n" + "2.Criar RMI.Eleicao\n" + "3.Alterar Propriedades de uma RMI.Eleicao\n"
                    ...
                    case "1": // "1.Registar Pessoas"
                        System.out.println(
                                "> Escolha uma das opcoes abaixo disponiveis de acordo com a Funcao da pessoa:\n"
                    ...
                    case "2": // "2.Criar Eleição"
                        ...
                        Date de_i = new Date(ano_i - 1900, mes_i - 1, dia_i, hora_i, minuto_i); // Date inicial que
                                                                                                // queremos criar
                        Date de_f = new Date(ano_f - 1900, mes_f - 1, dia_f, hora_f, minuto_f); // Date final que
                                                                                                // queremos criar
                        ...
                        if (check_f) {
                            System.out
                                    .println("Nao pode criar um eleicao cuja Date final seja anterior a Date inicial!");
                            break;
                        }
                        ...
                        eleicao = new Eleicao(ano_i, mes_i, dia_i, hora_i, minuto_i, ano_f, mes_f, dia_f, hora_f,
                                minuto_f, titulo, descricao, restricao, "", lista_lista_candidato, date_i, date_f);
                        while (true) {
                            try {
                    ...
                    case "3": // "3.Alterar Propriedades de uma Eleição"
                        ...
                        System.out.print("> Título da RMI.Eleicao: ");
                        ...
                        System.out.println(
                                "____________________________________________________________________________________________________________________________________\n");
                        eleicao = new Eleicao(ano_i, mes_i, dia_i, hora_i, minuto_i, ano_f, mes_f, dia_f, hora_f,
                                minuto_f, titulo, "", "", old_titulo, lista_lista_candidato, date_i, date_f);
                        while (true) {
                            try {
                    ...
                    case "4": // "4.Gerir Candidatos"
                        try {
                            ...
                            case "1": // "1.Adicionar Lista"
                                System.out.print("> Nome da RMI.Eleicao: ");
                                ...
                                System.out.println(
                                        "Vou guardar os dados de uma lista candidata da eleicao " + nome_eleicao);
                    ...
                    case "6": // "6.Consultar Informação de Voto"
                        System.out.println("RMI.AdminConsole - consulta_info_voto");
                        while (true) {
                            try {
                                // hmp ( { "RMI.HashMapPessoas" = mapp } | mapp = { num_cc = RMI.Pessoa } )
                    ...
                    case "7": // "7.Consultar Número de Eleitores"
                        System.out.println("RMI.AdminConsole - consulta_eleitores");
                        while (true) {
                            try {
                                // ( { dep = RMI.Mesa } )
                    ...
                    case "8": // "8.Consultar Resultados"
                        System.out.println("RMI.AdminConsole - consulta_resultados");
                        while (true) {
                            try {
                                // ( mape = { titulo = Eleição } )
                                ...
                                for (Map.Entry mapElement : mape.entrySet()) {
                                    ...
                                    if (verifica) {
                                        System.out.println("-------------------------------------------------\n"
                                                ...
                                                + ((double) e.getNum_votos_branco() * 100
                                        ...
                                        for (HashMap<String, ListaCandidato> llc : e.lista_lista_candidato) {
                                            ...
                                            for (Map.Entry<String, ListaCandidato> entry : llc.entrySet()) {
                                                System.out
                                                        .println(
                                                                "Numero de votos da lista de candidatos '"
                                                                        + entry.getValue().getNome_lista() + '\'' + ": "
                                                                        + entry.getValue().getNum_votos() + " ("
                                                                        + ((double) entry.getValue().getNum_votos()
                                                                                * 100 / (double) e.getNum_total_votos())
                                                                        + "%)");
                                            }
                                        }
                                    }
